...
public:
    HashTable(int length = 101, int (*f)(int) = defaultKey) : size(length), key(f), table(vector<Node*>(length, nullptr)) {}
    ...
    void remove(int x) {
        auto pp = &table[key(x) % size];
        while (*pp && x != (*pp)->data) pp = &(*pp)->next;
        if (*pp) {
            auto p = *pp;
            *pp = (*pp)->next;
            delete p;
